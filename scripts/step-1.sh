...
disable_hyprland_fallback_repo() {
    ...
    local hypr_pkgs=(
        ...
        hyprlock hyprpaper hyprpolkitagent hyprsunset hyprutils
        hyprwayland-scanner hyprland-qt-support hyprland-qtutils
    ...
    # Delete cached repodata for fallback repos so xbps stops seeing stale
    # ABI-linked metadata during the base update transaction.
    sudo find /var/db/xbps/ -maxdepth 1 \
        \( -name '*hyprland-void*' -o -name '*Makrennel*' \) \
        -delete 2>/dev/null || true

    # Always remove stale Hyprland extras before -Suv, because an installed
    # package like hyprsunset can remain broken even after the fallback repo
    # file is gone and will still abort the transaction.
...
# Networking, audio, Bluetooth, power
    xi_install NetworkManager NetworkManager-openvpn NetworkManager-openconnect NetworkManager-vpnc NetworkManager-l2tp network-manager-applet wpa_supplicant
    xi_install pipewire pipewire-pulse alsa-pipewire alsa-utils wireplumber wireplumber-elogind playerctl pavucontrol pamixer cava pulseaudio-utils pulsemixer rtkit
...
# Tailscale
    xi_install tailscale
    enable_service tailscaled

# Theme stuffs
    xi_install papirus-icon-theme

# Install fonts
    echo "Installing Fonts"
    cd "$builddir" || exit
    xi_install font-firacode-nerd
    xi_install font-jetbrains-mono-nerd
    xi_install noto-fonts-emoji
    xi_install terminus-font
    # Reload Font
    fc-cache -vf
    wait
...
# System Control Services
    ...
    # Enable Audio
        enable_service alsa
    # Enable Printer
        xi_install cups gutenprint cups-pk-helper nmap net-tools
